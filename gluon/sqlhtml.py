            ...
            if field.type == 'boolean':
                ...
                if f == '' or f is None:
                    ...
                    if not f:
                        ...
                        if 't_acogida' in formname:
                            ruta = os.path.join(current.request.folder,'Z/%s/Acogida/Documentos' % fields['dni'])
                        elif 't_demandante' in formname:
                            ruta = os.path.join(current.request.folder,'Z/%s/Club Trabajo/Club Trabajo %s/Solicitudes demandantes' % (fields['dni'], anyo))
                        else:
                            ruta = current.request.folder
                ...
                # Modificacion RBM; Prueba de upload con persona.
                import datetime
                fecha=datetime.datetime.now().strftime('%Y%m%d')
                anyo=fecha[:4]
                ...
                #    ruta = os.path.join(current.request.folder,'Z/%s/Club Trabajo/Club Trabajo %s/Solicitudes demandantes' % (fields['dni'], anyo))
                #     else:
                #         ruta = os.path.join(current.request.folder,'Z/%s/Club Trabajo/Club Trabajo %s/Solicitudes demandantes' % (fields['DM_CV'][0:9], anyo))
                #===============================================================
                elif ruta == None:
                    ruta = field.uploadfolder
